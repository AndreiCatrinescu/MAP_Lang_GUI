...
//        Statement err4 = Examples.exampleError4();
        ...
        for (var command: Examples.getAllExamples()) {
            commandIndex++;
            try {
                menu.addCommand(new RunExample("" + commandIndex, command.toString(), command));
            } catch (InterpreterError e) {
                menu.addCommand(new ShowErrorCommand("" + commandIndex, command.toString(), e.getMessage()));
            }
        }
//        menu.addCommand(new ExitCommand("exit", "exits the program"));
